...
func TestMakeDockerSocketTailer_not_docker(t *testing.T) {
	tf := &factory{}
	source := sources.NewLogSource("test", &config.LogsConfig{
		Type: "notdocker",
	})
	_, err := tf.makeDockerSocketTailer(source)
	require.ErrorContains(t, err, "only supported for docker")
}

func TestMakeDockerSocketTailer_success(t *testing.T) {
	...
	dst, err := tf.makeDockerSocketTailer(source)
	require.NoError(t, err)
	require.Equal(t, "abc", dst.(*dockerSocketTailer).containerID)
}
