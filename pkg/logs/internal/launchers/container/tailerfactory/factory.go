...
func New(sources *sources.LogSources, pipelineProvider pipeline.Provider, registry auditor.Registry) Factory {
	return &factory{
		sources:          sources,
...
func (tf *factory) MakeTailer(source *sources.LogSource) (Tailer, error) {
	return tf.makeTailer(source, tf.useFile, tf.makeFileTailer, tf.makeDockerSocketTailer)
}
...
func (tf *factory) makeTailer(
	...
	// depending on the result of useFile, prefer either file logging or socket logging,
	// but fall back to the opposite.
	if useFile(source) {
