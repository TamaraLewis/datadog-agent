...
// makeDockerSocketTailer makes a socket-based tailer for the given source, or returns
// an error if it cannot do so (e.g., due to permission errors)
func (tf *factory) makeDockerSocketTailer(source *sources.LogSource) (Tailer, error) {
	...
	// TODO: verify socket connectivity and return error here, to allow fallback
...
// dockerSocketTailer wraps the external docker Tailer to satisfy the `Tailer`
// interface, and to handle the erroredContainerID channel.
//
...
var _ Tailer = (*dockerSocketTailer)(nil)
